import logging

from root.shared.dead_letter import publish_dead_letter
from root.shared.parsing_failures import resolve_failure_reason
from root.shared.queues import EXTRACTION_URLS
from root.shared.rabbitmq import broker, faststream_app
from root.contracts.v1.pipeline_messages import ExtractionTask
from root.apps.workers.hunter_playwright.dependencies import pw_res
from root.apps.workers.hunter_playwright.logic.workflow import PlaywrightWorkflow

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    handlers=[logging.StreamHandler()],
)

logger = logging.getLogger(__name__)


@faststream_app.after_startup
async def _playwright_startup() -> None:
    await pw_res.init_all()
    logger.info(" [hunter] ресурсы инициализированы")


@faststream_app.on_shutdown
async def _playwright_shutdown() -> None:
    await pw_res.close_all()
    logger.info(" [hunter] ресурсы закрыты")


@broker.subscriber(EXTRACTION_URLS)
async def handle_extraction_task(msg) -> None:
    payload = msg.body.decode() if hasattr(msg, "body") else msg
    try:
        task = (
            ExtractionTask.model_validate_json(payload)
            if isinstance(payload, str)
            else ExtractionTask.model_validate(payload)
        )
        storage = await pw_res.get_storage()
        browser = await pw_res.get_browser()
        workflow = PlaywrightWorkflow(browser, storage)
        await workflow.run_extraction_task(task)
    except Exception as exc:
        reason = resolve_failure_reason(
            fallback="hunter_extraction_failed",
            exc=exc,
        )
        dlq_payload = (
            payload
            if isinstance(payload, dict)
            else {"raw_payload": str(payload)}
        )
        await publish_dead_letter(
            stage="hunter",
            reason=reason,
            payload=dlq_payload,
        )
        logger.critical(" [hunter] extraction task failed: %s", exc, exc_info=True)
        return
