# Общий разбор тела сообщений брокера для URL-задач (JSON RawUrlMessage или одна строка URL).
# Используется hunter и любыми подписчиками legacy-очередей, если они останутся.
...
class RawUrlPayloadError(ValueError):
    """Невалидное или пустое тело сообщения с URL."""
...
def parse_raw_url_message(raw: str) -> RawUrlMessage:
    ...
    text = raw.strip()
    if not text:
        raise RawUrlPayloadError("Пустое тело сообщения")

    if text.startswith("{"):
        try:
            data = json.loads(text)
        except json.JSONDecodeError as exc:
            logger.warning(" [payload] JSON не разобран: %s", exc)
